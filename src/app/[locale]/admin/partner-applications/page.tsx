    ...
    <div>
      <section className="gradient-hero py-12">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <p className="mt-1 text-emerald-200/80">{loading ? ta("loading") : t("applicationsReceived", { count: applications.length })}</p>
        </div>
      </section>
          ...
          {!loading && applications.length === 0 ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-cream-50 border-b border-gray-100"><tr>
                ...
                <th className="text-left px-6 py-3 font-semibold text-gray-600">{t("tier")}</th>
                ...
                {applications.map((a) => (
                  <tr key={a.id} className="hover:bg-cream-50">
                    ...
                    <td className="px-6 py-4 text-gray-500">{a.tierName}</td>
                    <td className="px-6 py-4">
                      <select
                        value={a.status}
                        disabled={updatingId === a.id}
                        onChange={(e) => updateStatus(a.id, e.target.value)}
                        className={`rounded-lg px-2 py-1 text-xs font-bold border-0 disabled:opacity-50 ${a.status === "approved" ? "bg-green-100 text-green-700" : a.status === "rejected" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                      >
