...
import { businessToursData, getTourDataBySlug, localizeTour } from "@/lib/tours";
import { formatNumber } from "@/lib/format";
import Button from "@/components/ui/Button";
import Card from "@/components/ui/Card";

export function generateStaticParams() {
  return businessToursData.map((t) => ({ slug: t.slug }));
}
      ...
      <section className="relative h-80 overflow-hidden bg-[var(--color-hero-bg)]">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="mx-auto max-w-7xl px-6 pb-8 w-full">
            <Link href="/services/business-tours" className="text-sm text-emerald-300 hover:text-emerald-200 mb-2 inline-block">{t("backToBusinessTours")}</Link>
        ...
        <div className="mx-auto max-w-7xl px-6 grid gap-10 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            <Card shadow="sm" bordered={false} className="p-6 sticky top-24">
              ...
              <Button href={"/services/business-tours/" + tour.slug + "/apply"} variant="primary" size="block">{t("applyNow")}</Button>
              <div className="mt-6">
