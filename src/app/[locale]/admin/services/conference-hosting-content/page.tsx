...
import { DEFAULT_CONFERENCE_HOSTING_CONTENT, type ConferenceHostingContent, type ConferenceHostingBenefit } from "@/lib/conference-hosting-content";
import Button from "@/components/ui/Button";
import Card from "@/components/ui/Card";

export default function AdminConferenceHostingContentPage() {
  const t = useTranslations("adminConferenceHostingContent");
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/conference-hosting-content")
      .then((res) => res.json())
      .then((data) => {
  ...
  function update(patch: Partial<ConferenceHostingContent>) {
    setContent((prev) => ({ ...prev, ...patch }));
  }

  function updateItem(index: number, patch: Partial<ConferenceHostingBenefit>) {
    setContent((prev) => ({
      ...prev,
      included: prev.included.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    }));
  }

  function addItem() {
    setContent((prev) => ({ ...prev, included: [...prev.included, { icon: "✨", title: "", desc: "" }] }));
  }

  function removeItem(index: number) {
    setContent((prev) => ({ ...prev, included: prev.included.filter((_, i) => i !== index) }));
  }
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      const res = await fetch("/api/admin/conference-hosting-content", {
        method: "PUT",
      ...
      <section className="gradient-hero py-12">
        <div className="mx-auto max-w-4xl px-6">
          ...
          <p className="mt-1 text-emerald-200/80">
            {t.rich("subtitle", { link: (chunks) => <Link href="/services/conference-forum-hosting" target="_blank" className="underline hover:text-white">{chunks}</Link> })}
          </p>
        </div>
      </section>
            ...
            <>
              ...
              <Card shadow="sm" bordered={false} className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="font-bold text-heading">{t("includedItems")}</h2>
