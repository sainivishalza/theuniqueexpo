    ...
    <div>
      <section className="gradient-hero py-12">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <h1 className="text-3xl font-extrabold text-white">{tour ? `${tour.title} — Registrations` : "Registrations"}</h1>
          <p className="mt-1 text-emerald-200/80">{loading ? "Loading..." : `${registrations.length} travelers registered`}</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          {!loading && !error && registrations.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl shadow-sm">
              ...
              <p className="text-gray-500">Traveler registrations for this tour will appear here.</p>
            </div>
          ) : !error && (
            <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-gray-50 border-b border-gray-100"><tr>
                <th className="text-left px-6 py-3 font-semibold text-gray-600">Traveler</th>
                ...
                {registrations.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-gray-900">{displayName(r)}</td>
