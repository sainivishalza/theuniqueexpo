...
interface ConferenceInquiry {
  id: string; name: string; email: string; company: string; eventType: string;
  expectedAttendees: string; status: string; createdAt: string;
}

const STATUSES = ["pending", "in-progress", "completed"];

export default function AdminConferenceInquiriesPage() {
  const t = useTranslations("adminConferenceInquiries");
  const ta = useTranslations("adminCommon");
  const { user, loading: authLoading } = useAuth();
  const [inquiries, setInquiries] = useState<ConferenceInquiry[]>([]);
  const [loading, setLoading] = useState(true);
  const [updatingId, setUpdatingId] = useState<string | null>(null);

  useEffect(() => {
    if (!user || user.role !== "admin") return;
    fetch("/api/admin/conference-inquiries")
      .then((res) => res.json())
  ...
  async function updateStatus(id: string, status: string) {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/admin/conference-inquiries/${id}`, {
        method: "PATCH",
          ...
          {!loading && inquiries.length === 0 ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-cream-50 border-b border-gray-100"><tr>
                ...
                <th className="text-left px-6 py-3 font-semibold text-gray-600">{t("eventType")}</th>
                <th className="text-left px-6 py-3 font-semibold text-gray-600">{t("expectedAttendees")}</th>
                ...
                {inquiries.map((a) => (
                  <tr key={a.id} className="hover:bg-cream-50">
                    ...
                    <td className="px-6 py-4 text-gray-500">{a.eventType}</td>
                    <td className="px-6 py-4 text-gray-500">{a.expectedAttendees || "—"}</td>
