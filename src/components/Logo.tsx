...
export default function Logo({ size = "default" }: { size?: "small" | "default" | "large" }) {
  ...
  return (
    <div className="flex items-center gap-2.5 group">
      <div className={`${s.box} rounded-[var(--radius-icon-xs)] gradient-brand flex items-center justify-center text-white font-bold shadow-sm`}>
        <svg viewBox="0 0 24 24" fill="none" className={`${size === "small" ? "w-4 h-4" : size === "large" ? "w-7 h-7" : "w-5 h-5"}`}>
          <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
      <div className={`${s.brand} font-extrabold tracking-tight text-heading leading-none whitespace-nowrap`}>
