    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/tours" className="mt-4 inline-block text-emerald-600 hover:underline">Browse all tours →</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8 bg-white rounded-2xl shadow-sm">
          ...
          <p className="text-gray-500 mb-6">Registration for <strong>{tour.title}</strong> is not currently open.</p>
          <Link href={`/tours/${tour.slug}`} className="inline-block rounded-xl gradient-brand px-6 py-3 text-sm font-semibold text-white">← Back to {tour.title}</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8 bg-white rounded-2xl shadow-sm">
          ...
          <p className="text-gray-500 mb-6">Your registration for <strong>{tour.title}</strong> has been received. Our travel team will review it and reach out shortly.</p>
          <Link href={`/tours/${tour.slug}`} className="inline-block rounded-xl gradient-brand px-6 py-3 text-sm font-semibold text-white">← Back to {tour.title}</Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="py-12 bg-gray-50 min-h-screen">
      <div className="mx-auto max-w-3xl px-6">
        <Link href={`/tours/${tour.slug}`} className="text-sm text-emerald-600 hover:text-emerald-700 mb-6 inline-block">← Back to {tour.title}</Link>
        <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Register for {tour.title}</h1>
        <p className="text-gray-500 mb-8">{tour.dates} • Tell us about your group and interests so we can tailor the trip.</p>
