...
interface SubsidyApplication {
  id: string; subsidyId: string; name: string; email: string; company: string; status: string; createdAt: string;
}

const STATUSES = ["pending", "in-progress", "completed"];

export default function AdminSubsidyApplicationsPage() {
  const t = useTranslations("adminSubsidyApplications");
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/subsidy-applications")
      .then((res) => res.json())
  ...
  async function updateStatus(id: string, status: string) {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/admin/subsidy-applications/${id}`, {
        method: "PATCH",
